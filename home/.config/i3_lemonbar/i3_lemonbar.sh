...
while read -r; do
  ...
  echo &&
    # restart alsactl if it exits
    until (stdbuf -oL alsactl monitor pulse | grep --line-buffered "Master Playback"); do
...
(cat "${panel_fifo}" | "$(dirname $0)"/i3_lemonbar_parser.sh |
  lemonbar -p -o -1 -f "${font}" -o 0 -f "${plfont}" -o -2 -f "${iconfont}" -a "${clickables}" -g "${geometry}" -B "${color_back}" -F "${color_fore}" | sh) &
