...
while read -r; do
  ...
  until (stdbuf -o0 udevadm monitor --udev --subsystem-match="backlight" --tag-match="seat"); do
...
lemonbar -p -o -3 -f "${font}" -o 0 -f "${plfont}" -o -4 -f "${iconfont}" -o -3 -f "${titlefont}" -a "${clickables}" -g "${geometry}" -B "${color_back}" -F "${color_fore}" | sh) &
