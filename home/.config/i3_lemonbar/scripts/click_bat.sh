...
if [ "${charging}" == "Unknown" ]; then
        (dunstify -a "clickBat" -u low -r "$msgId" "AC: $(acpi -b | awk '{print $4}' | tr -d ',')")
elif [ "${charging}" == "Discharging" ]; then
        (dunstify -a "clickBat" -u normal -r "$msgId" "Battery: $(acpi -b | awk '{print $4}' | tr -d ',')" "Time Remaining: $(acpi | awk '{print $5}')")
